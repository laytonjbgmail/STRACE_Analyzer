...
def my_median(local_list):
   ...
   local_list2 = list(local_list);
   
   # Find median value
   junk2 = len(local_list2);
   local_list2.sort();
   junk2 = len(local_list2);     # length of array
   junk1 = ( junk2%2 and 'odd' or 'even');
   if (junk1 == "odd"):
       # number is odd
       middle = junk2 / 2;
       My_Median2 = local_list2[middle];
   else:
       # number is even
       middle = (junk2 / 2) - 1;
       My_Median2 = ( local_list2[middle] + local_list2[middle+1]) / 2.0;
   # end if statement
   local_list2 = [];
   return My_Median2
# end def
...
#
# Determines if input is a number or not. It uses Python's exception handing to
#  discover this
#
def is_number(s):
   try:
...
#
# Find unit (file descriptor) from currentline
#   then find corresponding file in Open_Filename list
#   Result is returned in a dictionary:
#      result["unit"] contains the fd
#      result["match"] = i
#      result["filename"] cotnains the filename
#   Note: If result["match"] is negative then no filename was found and
#         the file name is returned as blank: "";
#
def get_fd_and_filename(currentline, Open_Filename, STRACE_LOG, LineNum, FUNC):
    
    # get fd: This assume it is the first thing after "(" in the strace output
    junk1 = currentline[1];
    junk2 = junk1.index("(");
    ...
    #       store data - otherwise, ignore the strace line because it's a socket
    #
    ...
    if (i < 0):                     # Change here to allow read to std
        junk1 = FUNC + ": No file attached to unit " + str(unit) + " - could be a socket\n";
        STRACE_LOG.write(junk1);
    ...
    result["unit"] = unit;
    result["match"] = i;
    result["filename"] = filename.lower();
